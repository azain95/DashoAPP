...
export const LeaveHistoryTab = ({ navigation }) => {
  ...
  const loadLeaves = async () => {
    setLoading(true);
    try {
      const { data } = await api.get('/requests?type=leave');
      setLeaves(data || []);
    } catch (error) {
  ...
  const renderItem = ({ item }) => {
    ...
    const leaveIcon = getLeaveTypeIcon(item.req_type);

    return (
      <Card style={styles.leaveCard}>
        <View style={styles.cardHeader}>
          <View style={styles.headerLeft}>
            <Icon name={leaveIcon} size={24} color={theme.colors.secondary} />
            <View style={styles.headerText}>
              <Text variant="titleMedium" style={styles.leaveType}>
                {item.req_type ? `${item.req_type.charAt(0).toUpperCase() + item.req_type.slice(1)} Leave` : 'Leave'}
              </Text>
  ...
  if (loading) {
    return (
      <View style={[styles.centerContainer, { backgroundColor: theme.colors.background }]}>
        <EmptyState icon="calendar-outline" title="Loading..." message="" />
      </View>
    );
  }
